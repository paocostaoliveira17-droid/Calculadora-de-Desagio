# Calculadora de Composto Misto

# ==========================================================
# Calculadora de Empréstimo com Juros Misto (Simples + Composto)
# - Prazo em dias (convertido para meses)
# - Mesma taxa mensal para simples e composto
# - Tarifas fixas descontadas
# Autor: ChatGPT (GPT-5)
# ==========================================================

def juros_simples(capital, taxa_mensal, meses):
    """Calcula juros simples"""
    return capital * taxa_mensal * meses

def juros_compostos(capital, taxa_mensal, meses):
    """Calcula juros compostos"""
    return capital * ((1 + taxa_mensal) ** meses - 1)

def calculadora_emprestimo_misto_dias():
    print("=== Calculadora de Empréstimo com Juros Misto (prazo em dias) ===\n")
    ...
    # --- Cálculo dos juros ---
    juros_s = juros_simples(valor_nominal, taxa, meses)
    juros_c = juros_compostos(valor_nominal, taxa, meses)
    juros_totais = juros_s + juros_c

    # --- Valor líquido recebido pelo tomador ---
    valor_liquido = valor_nominal - juros_totais - tarifa_total
    ...
    print(f"→ Juros simples acumulados: R$ {juros_s:,.2f}")
    print(f"→ Juros compostos acumulados: R$ {juros_c:,.2f}")
    print(f"→ Total de juros: R$ {juros_totais:,.2f}")
    print(f"→ Tarifas fixas descontadas: R$ {tarifa_total:,.2f}")

# Executar o programa
if __name__ == "__main__":
    calculadora_emprestimo_misto_dias()
